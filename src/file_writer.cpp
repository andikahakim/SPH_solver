...
/// Function to get velocity from particle class object
double get_velocity(SPH_particle p, int i) {
    /* Return ith element of velocity for particle p */
    return p.v[i];
}

/// Function to get position from particle class object
double get_position(SPH_particle p, int i) {
    /* Return ith element of position for particle p */
    return p.x[i];
}

/// Function to get height from particle class object
double get_height(SPH_particle p) {
    /* Return height for particle p */
    return p.x[1];
}

/// Function to get pressure from particle class object
double get_pressure(SPH_particle p) {
    /* Return pressure for particle p */
    return p.P;
}

/// Function to get desnity from particle class object
...
/// Function to get desnity from particle class object
double get_y_accel(SPH_particle p) {
    /* Return pressure for particle p */
    return p.a[1];
}
...
int write_file(const char* filename,
    ...
    fs << scalar_to_string("Height", particle_list, get_height);
    fs << scalar_to_string("Y-Acceleration", particle_list, get_y_accel);
    fs << "</PointData>\n";
    fs << "<Points>\n";
